...
def delete_handle(data, command):
    ret = 'OK'
    ret_msg = ''
    ...
    try:
        ...
        # find the device
        cur = conn.cursor()
        cur.execute("SELECT CIP FROM devices VALUES WHERE NAME = ?", (data['name']))
        conn.commit()
        ...
        if row == None:
            ...
            send_client_command(row[0], command)
            # checks out; delete device
            conn.execute("DELETE FROM devices VALUES WHERE NAME = ?", (data['name']))
            conn.commit()
    except Exception as e:
        # Roll back any change if something goes wrong
        conn.rollback()
